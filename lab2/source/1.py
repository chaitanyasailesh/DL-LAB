...
x_train, x_valid, y_train, y_valid = train_test_split(data.iloc[:,0:13], data.iloc[:,13], test_size=0.2, random_state=87)
np.random.seed(155)
...
history = Model.fit(x_train, y_train, batch_size=64, epochs=25, shuffle=True, verbose=0, validation_data=(x_valid, y_valid), callbacks=[TensorBoardColabCallback(tbc)])
